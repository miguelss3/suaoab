...
const DossieAluno = ({ aluno, onClose }: { aluno: any, onClose: () => void }) => {
  ...
  const abrirEdicaoMeta = (idx: number, meta: any) => {
    setMetaEditandoIdx(idx); setEditMetaTitulo(meta.atividade || ""); setEditMetaDescricao(meta.orientacoes || "");
    setEditMetaLink(meta.link || ""); setEditMetaArquivo(null);
    setEditMetaPrazo(meta.data_sugerida ? new Date(meta.data_sugerida).toISOString().split('T') : "");
  };
      ...
      <div className="bg-card border border-border w-full max-w-5xl max-h-[95vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto space-y-8 custom-scrollbar">
            ...
            <div className="lg:col-span-3 bg-muted/10 p-5 rounded-xl border border-border">
              ...
              <div className="flex gap-3 items-center">
                <div className="flex-1 relative">
                  <input type="file" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={e => setNovaMetaArquivo(e.target.files?. || null)} />
          ...
          <div className="space-y-3">
            {(() => {
              let metaCounter = 1;
              return (aluno.metas || []).map((m: any, idx: number) => {
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 bg-muted/10 p-4 rounded-lg border border-border">
                ...
                <div className="relative w-full">
                  <input type="file" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={e => setEditMetaArquivo(e.target.files?. || null)} />
                         ...
                         <div key={i} className="p-5 rounded-xl border-2 border-border bg-background shadow-sm space-y-4">
                            ...
                            <div className="grid md:grid-cols-[1fr_150px] gap-4">
                              ...
                              <div className="space-y-1"><Label className="text-[10px] uppercase font-black text-muted-foreground">Prazo</Label>
                                <Input type="date" value={m.data_sugerida ? m.data_sugerida.split('T') : ''} onChange={(e) => { const val = e.target.value ? new Date(e.target.value + "T12:00:00").toISOString() : ""; handleEditPreviewMeta(i, 'data_sugerida', val); }} />
                              </div>
                            </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-4 mt-2 p-3 bg-muted/10 rounded-lg border border-dashed border-border">
                              ...
                              <div className="space-y-1"><Label className="text-[10px] uppercase font-black text-muted-foreground">Anexo</Label>
                                <div className="relative w-full"><input type="file" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={(e) => handleEditPreviewMeta(i, 'arquivo_file', e.target.files?. || null)} />
